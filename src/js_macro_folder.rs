...
impl<'a> JsMacroFolder<'a> {
  ...
  fn update_msg_descriptor_props(&self, expr: Box<Expr>) -> Box<Expr> {
    if let Expr::Object(obj) = *expr {
      ...
      let context_val =
        get_object_prop(&obj.props, "context").and_then(|prop| get_expr_as_string(&prop.value));
      ...
      if let Some(prop) = message_prop {
        ...
        if !id_prop.is_some() {
          new_props.push(create_key_value_prop(
            "id",
            generate_message_id(&parsed.message_str, &(context_val.unwrap_or_default())).into(),
          ))
        }
...
impl<'a> Fold for JsMacroFolder<'a> {
  fn fold_expr(&mut self, expr: Expr) -> Expr {
    ...
    if let Expr::TaggedTpl(tagged_tpl) = &expr {
      ...
      if is_t {
        return Expr::Call(
          self.create_i18n_fn_call_from_tokens(callee, self.ctx.tokenize_tpl(&tagged_tpl.tpl)),
        );
      }
    }
    ...
    if let Expr::Call(call) = &expr {
      if let Some(_) = match_callee_name(&call, |n| self.ctx.is_define_message_ident(n)) {
        if call.args.len() == 1 {
          let descriptor =
            self.update_msg_descriptor_props(call.args.clone().into_iter().next().unwrap().expr);
