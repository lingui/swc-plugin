...
impl MacroCtx {
  ...
  pub fn is_define_message_ident(&self, ident: &Ident) -> bool {
    return self.is_lingui_ident("defineMessage", &ident) || self.is_lingui_ident("msg", &ident);
  }
    ...
    for spec in &imp.specifiers {
      if let ImportSpecifier::Named(spec) = spec {
        if let Some(ModuleExportName::Ident(ident)) = &spec.imported {
          self
            .imports_id_map
            .insert(ident.sym.clone(), spec.local.to_id());
          self
            .imports_id_map_inverted
            .insert(spec.local.to_id(), ident.sym.clone());
        } else {
          self
            .imports_id_map
            .insert(spec.local.sym.clone(), spec.local.to_id());
          self
            .imports_id_map_inverted
            .insert(spec.local.to_id(), spec.local.sym.clone());
        }
      }
    }
  }
