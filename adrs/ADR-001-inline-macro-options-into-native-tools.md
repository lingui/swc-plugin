# ADR-001: Inline macro options source into @lingui/native-tools
...
The `@lingui/native-tools` package (`packages/native-tools`) ships a native binary (compiled from Rust via NAPI-RS) that includes the macro transformation logic from `lingui_macro`. The TypeScript layer in `@lingui/native-tools` needs to reuse type definitions (`LinguiMacroOptions`) and a mapping function (`mapOptions`) that are defined in the `@lingui/swc-plugin` package — they describe and configure the behavior baked into the native binary.
...
Inline the relevant source from `packages/lingui-macro/src-js` into the `@lingui/native-tools` build using a directory symlink (`src-js/macro-src` -> `../../lingui-macro/src-js`). TypeScript compiles both files into `dist/`, producing a self-contained package with no cross-package import paths.
...
- **No runtime dependency on `@lingui/swc-plugin`.** `@lingui/native-tools` does not need to declare it as a dependency.
